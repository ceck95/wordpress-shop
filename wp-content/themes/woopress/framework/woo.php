	...
	function et_hidden_sidebar_btn() {
		?>
			<div id="st-trigger-effects" class="column pull-left">
				<button data-effect="hide-filters-block" class="btn filled small-btn"><?php _e('Show Filter', ETHEME_DOMAIN); ?></button>
			</div>
		<?php
	}
}
			...
			<div class="shopping-container" <?php if(etheme_get_option('favicon_badge')) echo 'data-fav-badge="enable"' ?>>
				<div class="shopping-cart-widget" id='basket'>
					...
					<div class="cart-bag">
						...
						<span class='shop-text'><?php _e('Giỏ hàng', ETHEME_DOMAIN) ?>:</span> <span class="total"><?php echo $woocommerce->cart->get_cart_subtotal(); ?></span>
					</a>
				</div>
...
if(!function_exists('etheme_cart_items')) {
	function etheme_cart_items ($limit = 3) {
        ...
        if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) {
          ?>
			<p><?php _e('Sản phẩm đưọc thêm gần đây', ETHEME_DOMAIN); ?></p>
			<ul class='order-list'>
          <?php
            $counter = 0;
        ...
        <?php   
            ...
            echo '<p class="empty a-center">' . __('Không có sản phẩm trong giỏ hàng.', ETHEME_DOMAIN) . '</p>';
        }
        ...
        if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) {
          ...
          ?>
			<p class="small-h pull-left"><?php echo __('Tổng cộng', ETHEME_DOMAIN); ?></p>
			<span class="big-coast pull-right">
				<?php echo $woocommerce->cart->get_cart_subtotal(); ?>
			</span>
			<div class="clearfix"></div>
			<div class='bottom-btn'>
				<a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" class='btn text-center border-grey'><?php echo __('Xem giỏ hàng', ETHEME_DOMAIN); ?></a>
				<a href="<?php echo $woocommerce->cart->get_checkout_url(); ?>" class='btn text-center big filled'><?php echo __('Thanh toán', ETHEME_DOMAIN); ?></a>
			</div>
